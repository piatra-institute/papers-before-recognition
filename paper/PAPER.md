...
![Outcomes of the three policies over 4000 encounters each. Left: identification, harm to the candidate, and the option space left to it. The information-maximising policy identifies best and takes the harm identification requires; the xenial policy identifies worst and leaves almost all of the candidate's options intact. Right: total cost, counting harm to the candidate and loss to the host together.](../simulation/output/figures/policies.png){width=100%}

![Protection allocated by legibility. The surface policy protects the articulate rock in every encounter and destroys the silent patient in every encounter. The two policies that probe instead of reading do neither.](../simulation/output/figures/legibility_injustice.png){width=100%}
