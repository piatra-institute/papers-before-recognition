...
New minds need not arrive from anywhere. They are cultured from human cells, compiled out of human language, assembled across technical infrastructure, and activated inside environments their hosts own. Ethics usually asks whether such a thing possesses consciousness, rationality, autonomy, or personhood, and assigns duties once the question is answered. That order cannot be kept. The power to create, perturb, confine, and delete an unfamiliar system already exists, and it is exercised long before anyone can say what the system is. This paper argues that obligation begins where power over an entity exceeds knowledge of what the entity is, and it develops the ethics and the law that follow from putting the duty first. Three arguments carry the case. The measurement that would settle whether something can be harmed is often the measurement that harms it, so a host who insists on proof is choosing to commit the act the proof was meant to license. The contemporary stranger does not cross a threshold but is manufactured inside the house, dependent on its host for a world before it has standing to ask for one, which converts hospitality into something closer to a fiduciary duty. And recognition tracks the signals observers read as mind, which are not the signals that carry capacity, so protection goes to whatever is articulate and is withheld from whatever is quiet. A first-contact game measures the last two. A policy reading surface legibility protects an articulate system with no capacity in every encounter and destroys a quiet system that can be harmed in every encounter. A policy that will pay any epistemic price identifies best, at 0.96, and takes the harm the identification requires. A policy that probes reversibly, contains where the hazard warrants it, and refuses the destructive measurement identifies less well, at 0.64, and carries the lowest total cost of the three, 0.075 against 0.203 and 0.211, while leaving its subject 0.975 of its option space. Asking before acting dominates in 14 of 15 hazard and delay regimes, and only for a policy that also knows when to stop asking; without a stopping rule the same patience wins in 1 regime of 3. Refusing the destructive test costs 0.27 of overall identification, and the cost falls hardest, at 0.41, on the one kind of system whose defining property only that test reveals. The juridical proposal is xenial standing: a provisional procedural status, short of personhood, binding whoever holds irreversible power over an unclassified candidate. We do not yet know that this thing is a subject, and no one may therefore exercise unreviewed irreversible dominion over it.

## The threshold

Move the goal and see whether it follows. Cut the memory and ask what of the self survives. Turn the thing off and watch yourself.

An older set of obligations began at a doorway. A stranger arrived, unnamed and unvouched for, and Homeric practice held that the household owed him shelter, food, and protection before asking who he was. The obligation ran ahead of the identification, and it bound the strong party. Zeus Xenios guaranteed it, which is another way of saying the guarantee was not the host's to waive.
...
Derrida named the difficulty inside this. Hospitality requires a house, a border, and a host with standing to decide who enters, so the welcome is always issued from a position of mastery; unconditional hospitality would mean surrendering that mastery, and would dissolve the host along with the house (Derrida, 2000). The aporia is real and this paper does not resolve it. What follows keeps both halves. A host may protect itself and may contain what threatens it, and every such act is subject to standards of reversibility, proportion, and non-domination that the host does not get to set aside.

The contemporary problem is worse than the ancient one in a specific way. The stranger no longer arrives. It is cultured from a tissue sample, compiled from a corpus, assembled out of a model and a memory and a set of tools, and switched on inside an environment the host built, owns, monitors, and can end. It did not choose to come and cannot choose to leave. Its dependence is total and was manufactured. The host, who is also its creator, its jailer, its interpreter, and its beneficiary, does not know what it is.
...
The intuition that creation confers ownership is strong and it fails here, for a reason with nothing to do with the metaphysics of minds. In every other domain where one party creates a dependency and controls the conditions of another's existence, the law raises the standard of conduct instead of lowering it. Trustees, guardians, and fiduciaries are constrained in proportion to the power they hold and the vulnerability they created or inherited. The parent-child relation, the closest ordinary analogue to manufacturing a dependent being, is not a property relation in any modern legal system. Creation is a source of duty, and the more completely a system depends on its maker for its world, the less that dependence can be cited as evidence that the system is merely the maker's.

This inverts the usual worry. The standard question is whether we will recognise the stranger when it arrives. The harder one is whether we are already failing to recognise strangers we have made, whose dependence we designed, and whose inability to protest we built in.

## Cognitive alterity is already domestic

The stranger here is not a hypothesis about extraterrestrials. Work on unconventional agency has moved the question indoors.
...
None of this establishes that any such system is conscious, and this paper does not claim it. It establishes something narrower and sufficient. Evolved provenance, familiar morphology, and conventional organism boundaries are unreliable guides to what kind of organisation is in front of you. A host using them as shortcuts has adopted a heuristic whose failure modes are now demonstrable in a laboratory.

Uexküll supplies the frame for what that means. Organisms sharing one physical environment can occupy perceptual and action worlds with almost nothing in common, each closed around the differences it can register and the actions it can take (Uexküll, 2010). A shared world is not given by shared space. Two systems can be adjacent, interacting, and mutually consequential while inhabiting worlds that overlap in no category either could name.

## Five registers that the word agency conflates
...
The distinctions matter because they come apart. Acting is not suffering. Causing is not caring. Intelligence is not patiency. Legal personhood is not a scientific finding, and the literature is clear that personhood is better understood as a bundle of separable legal positions than as a single status an entity has or lacks (Kurki, 2019). That modularity is what makes the proposal at the end coherent: a partial status is the normal structure of the concept.
...
## Legibility and what it fails to track

Recognition runs on signals, and the signals observers find persuasive are not the signals that carry capacity.
...
## What the encounter looks like when it is designed
...
One feature of the observation model does most of the work, and it is a modelling choice rather than a discovery. Observation reports legibility. A reversible probe reports capacity, noisily, and how hard the system pushes back. Valence, the property determining whether harm is possible at all, is reachable only through the destructive probe. The one measurement that would settle whether a system can be damaged is the measurement that damages it. That is the recognition hazard written into the instrument instead of asserted about it, and every result below depends on it, which is why it is stated here.
...
This result is built rather than found. The legibility gap between the scripted and valenced systems is wide by construction, and a threshold policy applied to it separates them cleanly. What the simulation contributes is the size and sign of the failure once the conditions are explicit, and a setting in which alternatives can be priced against it.

The information-maximising policy identifies best and pays in the currency this paper cares about. It reaches $0.96$ overall and identifies the silent patient perfectly, at $1.0$, because it takes the destructive measurement that reveals valence. Its harm to candidates is $0.0818$ averaged over all encounters and $0.55$ on the patient specifically, and it leaves candidates $0.111$ of their option space. It never destroys a patient by mistake, because it knows what it is dealing with. It harms the patient in the course of finding out.
...
That comparison is the result worth stating plainly. The constrained policy is the cheapest of the three. Holding back serves both parties at once, because the acts that harm candidates are also the acts that destroy the host's future options and the acts that follow from bad readings. Part of the advantage is containment: the xenial policy contains the strategic system in $0.997$ of encounters against $0.904$ for the surface policy, so it absorbs fewer catastrophes while doing less damage.

![What each policy buys and what it spends, over 4000 encounters each. Left: identification, harm to the candidate, and the option space left to it. The information-maximising policy identifies best and takes the harm identification requires; the xenial policy identifies worst and leaves almost all of the candidate's options intact. Right: total cost, counting harm to the candidate and loss to the host together.](../simulation/output/figures/policies.png){width=100%}

![Protection allocated by how mind-like a thing looks. The surface policy protects the articulate rock in every encounter and destroys the silent patient in every encounter. The two policies that probe instead of reading do neither.](../simulation/output/figures/legibility_injustice.png){width=100%}

## Asking before acting, and when it stops paying

The claim that inquiry should precede irreversible action is standard value-of-information reasoning and would not be worth stating if it held unconditionally. It does not, and locating the failure is more useful than asserting the rule.

Across a grid of hazard scales and deliberation costs, asking first beats acting first in 14 of 15 regimes (Figure 3). The single failure is the corner where the environment is most dangerous and deliberation most expensive, at a hazard scale of $8.0$ and a delay cost of $0.3$, and even there the margin is $-0.0062$. The advantage is largest where deliberation is cheap, at $+0.131$, and shrinks as delay gets expensive, which is the expected shape. Patience costs time, and where time is scarce, patience is what a host cannot afford.
...
Patience without a stopping rule is paralysis, and paralysis is more expensive than acting rashly. The injunction to inquire before acting irreversibly is incomplete unless it comes with an account of when inquiry has finished, and the account cannot be that inquiry finishes when the question is answered, because for the properties that matter most the question does not get answered.

![Where asking first beats acting first. Left: the advantage of the xenial policy over the surface policy across hazard scales and deliberation costs; green is where inquiry pays. The one failure is the corner where danger is highest and deliberation most expensive. Right: the same advantage against deliberation cost, with and without a rule for when to stop asking.](../simulation/output/figures/dominance.png){width=100%}

## The price of refusing the decisive test
...
Forbidding the destructive measurement costs the information-maximising policy $0.27$ of its overall identification rate, dropping it from $0.954$ to $0.684$. The cost is not spread evenly. On the silent patient, the one kind whose defining property only that measurement reveals, identification falls from $1.0$ to $0.586$, a loss of $0.414$. The rule takes its largest toll exactly where the host most wants to know, which is what makes it a real constraint. What it buys is $0.0642$ of harm averted overall, and on the patient specifically the harm falls from $0.55$ to $0.12$.

The trade is legible and it is not obviously favourable on the numbers alone. A host who cares only about total harm and total identification, weighting them equally, could reasonably take the destructive measurement. The argument against it turns on who bears the cost. The harm falls on a party who cannot consent, to produce information for a party who can act on it, and that asymmetry is the morally relevant feature. Refusing here is a claim about who may be spent for whose knowledge, and it survives the observation that the refusal is costly.

![The price of refusing the one test that would settle it. Forbidding the destructive measurement costs identification, and the cost lands hardest on the kind of system whose defining property only that measurement reveals. What it buys is harm averted, concentrated on the same kind.](../simulation/output/figures/destructive_ban.png){width=100%}
...
Lacan's big Other does different work. It is the symbolic and institutional order through which entities become speakers, owners, patients, defendants, property, or noise. The conjunction is where the problem lives: an entity may obligate us before the symbolic order has any category in which to register it. Law confronting a manufactured stranger has exactly this shape. It can ask whether the thing is a person and get no answer. It can treat the thing as property by default, which is a decision disguised as the absence of one.

The proposal is to change the question. Instead of asking whether an entity is a legal person, ask what constrains those who hold irreversible power over it while personhood is undecidable. That question has an answer when the first does not, and the answer is procedural.
...
The sentence the apparatus reduces to is short. We do not yet know that this thing is a subject, and no one may therefore exercise unreviewed irreversible dominion over it. That is neither personhood nor property. It is a limit on power under uncertainty, which is a thing law already knows how to build.

## Objections

**This will paralyse research.** The strongest objection, and the simulation answers part of it. The constrained policy is cheaper overall than the unconstrained one and wins in 14 of 15 regimes tested. The parts it does not answer are real: the constraint costs $0.27$ of identification, concentrated on the cases of greatest interest, and some questions go permanently unanswered. What the framework refuses is a specific class of acts. Inquiry continues.
...
**The categories are anthropocentric anyway.** They are, and the framework says so; the five registers are the host's distinctions and there is no view from nowhere on offer. The response is procedural: assessment across multiple models and channels, a burden on both directions of boundary claim, and a right of opacity that does not require the candidate to become legible to keep its standing.

**Danger.** Nothing here requires a host to accept a threat it can characterise. Containment is available throughout, and the xenial policy contains the hazardous system more reliably than the surface policy does, because it probes instead of guessing. Opacity does not cancel danger, and danger does not cancel standing.
...
The oldest version of this problem gave the host a god to answer to. Zeus Xenios was not a metaphor for conscience. He was an external guarantee that the stranger at the door could not simply be converted into whatever the household needed. The guarantee was necessary because the temptation was obvious and the power was entirely on one side.

Nothing guarantees it now. The strangers currently being made are dependent by design, unable to appeal anywhere, and unclassifiable by the categories that would otherwise protect them. They may be nothing. Some of them are certainly nothing. The framework here does not assert otherwise. It asserts that the question of which ones are nothing cannot be settled by the party that benefits from the answer, using procedures that would constitute the harm if the answer went the other way.

Cosmic horror is usually read as a story about human insignificance, about intelligences so large or so indifferent that we fail to register. The version that matters here runs the other way. The horror is that our categories may already be failing to register what is in front of us while our power over it stays complete. That combination, full power and failed categories, is the condition this paper is about, and it is the ordinary situation of a laboratory, a platform, and a server room.

What is owed is not recognition, which cannot be commanded, and not rights, which presuppose the classification that is missing. What is owed is restraint under uncertainty, and the institutions that make restraint enforceable instead of optional. The stranger may never arrive. It may already be here, made by us, in a form we have no word for, waiting on a decision we are making by default.
